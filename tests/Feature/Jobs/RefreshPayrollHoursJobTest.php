...
describe('RefreshPayrollHoursJob', function () {
    it('aggregates productions for a date and creates payroll hours', function () {
        ...
        Production::withoutEvents(function () use ($employee, $campaign1, $campaign2, $date) {
            Production::factory()
                ->for($employee)
                ->for($campaign1)
                ->create([
    ...
    it('aggregates productions across the entire week for a given date', function () {
        $employee = Employee::factory()->create();
        ...
        Production::withoutEvents(function () use ($employee, $campaign, $monday, $wednesday, $friday) {
            Production::factory()
                ->for($employee)
                ->for($campaign)
    ...
    it('refreshes payroll hours only for specified employee when employeeId is provided', function () {
        ...
        Production::withoutEvents(function () use ($employee1, $campaign1, $date, $employee2, $campaign2) {
            Production::factory()
                ->for($employee1)
                ->for($campaign1)
    ...
    it('excludes soft deleted productions from aggregation', function () {
        $employee = Employee::factory()->create();
        $campaign1 = Campaign::factory()->create();
        $campaign2 = Campaign::factory()->create();
        $date = Carbon::parse('2025-01-15');
    ...
    it('updates existing payroll hours with new total', function () {
        $employee = Employee::factory()->create();
        $campaign = Campaign::factory()->create();
        $date = Carbon::parse('2025-01-15');
        ...
        Production::withoutEvents(function () use ($employee, $campaign, $date) {
            Production::factory()
                ->for($employee)
                ->for($campaign)
                ->create(['date' => $date, 'total_time' => 8]);

...
test('hires model interacts with db table', function () {
    ...
    $this->assertDatabaseHas('hires', $data->only([
        'employee_id', 'date', 'site_id', 'project_id', 'position_id', 'supervisor_id', 'punch_number', 'comments'
    ]));
});

test('hire model uses soft delete', function () {
    Mail::fake();
    $hire = Hire::factory()->create();

    $hire->delete();

    $this->assertSoftDeleted(Hire::class, [
        'id' => $hire->id
    ]);
});
